...
def combined_loss(denoised, target, class_output, class_target, mask, threshold, alpha=1, beta=1, gamma=0.001):
    # MSE for signal
...
#    return total_loss
    return total_loss, mse_signal, mse_noise, class_loss
...
val_mse_noise = []
val_class_loss = []
far_threshold = 0.5  # Training FAR threshold
val_far_threshold = 0.5  # Validation FAR threshold
...
#                noise_class_outputs.append(class_output.squeeze().cpu().numpy()[noise_mask])
                noise_class_outputs.extend(class_output.squeeze().cpu().numpy()[noise_mask].tolist())
    # Compute 1% FAR threshold
    #noise_class_outputs = np.concatenate(noise_class_outputs, axis=0)
    ...
    for inputs, targets, mask, labels in train_loader:
        ...
        total_loss, mse_signal, mse_noise, class_loss = combined_loss(denoised, targets, class_output, class_target, mask, far_threshold)
    ...
    train_class_loss_epoch = running_train_class_loss / len(train_loader)
    train_losses.append(train_loss)
    train_mse_signal.append(train_mse_signal_epoch)
    train_mse_noise.append(train_mse_noise_epoch)
    train_class_loss.append(train_class_loss_epoch) 
    ...
    with torch.no_grad():
        for inputs, _, _, labels in val_loader:
            ...
            if noise_mask.any():
                ...
                val_noise_class_outputs.extend(class_output.squeeze().cpu().numpy()[noise_mask].tolist())
...
#    val_noise_class_outputs = np.concatenate(val_noise_class_outputs, axis=0)
    ...
    with torch.no_grad():
        for inputs, targets, mask, labels in val_loader: 
            ...
            denoised, class_output = model(inputs)
            total_loss, mse_signal, mse_noise, class_loss = combined_loss(denoised, targets, class_output, class_target, mask, far_threshold)  # Use training threshold for loss
    ...
    val_class_loss_epoch = running_val_class_loss / len(val_loader)
    val_losses.append(val_loss)
    val_mse_signal.append(val_mse_signal_epoch)
    val_mse_noise.append(val_mse_noise_epoch)
    val_class_loss.append(val_class_loss_epoch)     
    ...
    if epoch % 3 == 0:
        ...
        print(f"Epoch {epoch + 1}/{num_epochs}, Training Loss: {train_loss:.3e}, Validation Loss: {val_loss:.3e}")
        print(f"Training MSE Signal: {train_mse_signal_epoch:.3e}, MSE Noise: {train_mse_noise_epoch:.3e}, Class Loss: {train_class_loss_epoch:.3e}")
        print(f"Validation MSE Signal: {val_mse_signal_epoch:.3e}, MSE Noise: {val_mse_noise_epoch:.3e}, Class Loss: {val_class_loss_epoch:.3e}")
...
val_class_loss = np.array(val_class_loss)
