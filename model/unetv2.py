...
class UNet(nn.Module):
    def __init__(self, input_channels=1, output_channels=1, size_filter_in=16, dropout_prob=0.3):
    ...
    def _initialize_weights(self, layer):
        if isinstance(layer, nn.Conv2d):
            # Kaiming initialization for Conv2d layers with Leaky ReLU
            nn.init.kaiming_normal_(layer.weight, mode='fan_in', nonlinearity='leaky_relu', a=0.01)
        elif isinstance(layer, nn.ConvTranspose2d):
            # Kaiming initialization for ConvTranspose2d layers with Leaky ReLU
            nn.init.kaiming_normal_(layer.weight, mode='fan_in', nonlinearity='leaky_relu', a=0.01)
        elif isinstance(layer, nn.Linear):
            # Kaiming initialization for Linear layers
            nn.init.kaiming_normal_(layer.weight, mode='fan_in', nonlinearity='linear')
        elif isinstance(layer, nn.BatchNorm2d):
            # BatchNorm layer initialization (if you're using BatchNorm, which is not in your current U-Net code)
