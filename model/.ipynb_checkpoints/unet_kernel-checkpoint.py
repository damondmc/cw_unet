...
class conv_block(nn.Module):
    ...
    def __init__(self, in_channels, out_channels):
        ...
        self.conv = nn.Sequential(
            nn.Conv2d(in_channels, out_channels, kernel_size=(3, 5), stride=1, padding=(1, 2), bias=True),
            nn.BatchNorm2d(out_channels),
            nn.LeakyReLU(inplace=True),
            nn.Conv2d(out_channels, out_channels, kernel_size=(3, 5), stride=1, padding=(1, 2), bias=True),
            nn.BatchNorm2d(out_channels),
            nn.LeakyReLU(inplace=True)
        )
