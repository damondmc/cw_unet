...
label = 'fast_UNET_weight_alpha{}beta{}_{}Hz_D{}-{}_T{}_Tsft{}_ndata{}_step{}_ndata{}_th{}'.format(alpha, beta, f0, int(max_train_levels[0]), int(max_train_levels[-1]), int(obsTime//86400), Tsft, n_data, n_step, n_data*1000, threshold)
...
filename = '/scratch/kriles_root/kriles0/damoncht/unet_f/data/validation/{0}Hz_H1L1_D0-{4}_{1}x{2}_{3}s_4c_traindata_n{5}_seed0.npz'.format(f0, size[0], size[1], Tsft, 120, 400)
targets = np.load(filename, allow_pickle=True)['clean_image']
masks = np.load(filename, allow_pickle=True)['signal_mask']

data = normalize(max_train_levels[0] * noise + targets)
targets = normalize(targets)

signal_dataset = load_signal_datasetv2(data, targets, masks, max_train_levels[0])
...
for epoch in tqdm(range(num_epochs)):   
    if epoch % n_step == 0:
        ...
        for i in range(n_data):  # Iterate over n datasets
            seed = ((epoch // n_step) * n_data + i) % 150  # Generate unique seed for each dataset
            ...
            filename = '/scratch/kriles_root/kriles0/damoncht/unet_f/data/{0}Hz/{0}Hz_H1L1_D0-{4}_{2}x{3}_{5}s_4c_traindata_n1000_seed{1}.npz'.format(f0, seed, size[0], size[1], 20, Tsft)
            ...
            signal_data = load_signal_datasetv2(data, targets, masks, max_train_levels[0])
            
            datasets.append(signal_data)

        seed = (epoch // n_step) % 148 + 150  # Generate unique seed for each dataset
