...
filename = '/scratch/kriles_root/kriles0/damoncht/unet_f/data/pure_noise/H1L1_256x128_7200s_4c_purenoise_n1000_seed299.npz'
noise = np.load(filename, allow_pickle=True)['pure_noise'][:400]

# make new training data for every 5 steps
with Pool(processes=num_cpus) as pool:
    noise = pool.starmap(normalize, [
        (d,)
        for d in noise
    ])
...
params = genSampleParam(max_val_levels, f1min, f1max, 200)
...
signal_dataset = crop_signal_img(_d1, _d2, freq_size=freq_size, threshold = threshold)
...
for epoch in tqdm(range(num_epochs)):   
    if epoch % (n_data*n_step) == 0: 
        for i in range(n_data):
            if threshold != 50:
                filename = '/scratch/kriles_root/kriles0/damoncht/unet_dyn_fastnoise/data/{0}Hz/{0}Hz_H1L1_D0-35_256x128_7200s_4c_traindata_n1000_seed{1}_th{2}_norm.npz'.format(f0, n_data*(epoch//(n_data*n_step))+i, threshold)
            else:
                filename = '/scratch/kriles_root/kriles0/damoncht/unet_dyn_fastnoise/data/{0}Hz/{0}Hz_H1L1_D0-35_256x128_7200s_4c_traindata_n1000_seed{1}.npz'.format(f0, n_data*(epoch//(n_data*n_step))+i)
                
            print("Using {}".format(filename))
            data = np.load(filename, allow_pickle=True)
            
            clean_signal = data['clean_image']
            mask_data = data['signal_mask']
...
#             noise = pool.starmap(simNoise, [
#                 (1, Tsft, size, 2, False)
#                 for _ in range(clean_signal.shape[0])
#             ])

#         noise = np.stack(noise)   
        filename = '/scratch/kriles_root/kriles0/damoncht/unet_f/data/pure_noise/H1L1_256x128_7200s_4c_purenoise_n1000_seed{0}.npz'.format(n_data*(epoch//(n_data*n_step))+i)
        noise = np.load(filename, allow_pickle=True)['pure_noise']  
        # Process pure noise data
        images = max_train_levels[0] * noise + clean_signal
        ...
        signal_dataset = load_signal_dataset(images, clean_signal, mask_data, max_train_levels[0])
...
#         print("Using {} noise images".format(noise.shape[0]))
        ...
        filename = '/scratch/kriles_root/kriles0/damoncht/unet_f/data/pure_noise/H1L1_256x128_7200s_4c_purenoise_n1000_seed{0}.npz'.format(100+n_data*(epoch//(n_data*n_step))+i)
        noise = np.load(filename, allow_pickle=True)['pure_noise']  
